...
class MS_ extends MS365 {
    public function __construct()
    {
        $client_id = '734ef928-d74c-4555-8d1b-d942fa0a1a41';
        $client_secret = ':EK[e0/4vQ@mQgma8LmnWb6j4_C1CSIW';
        ...
        if (getConfig('siteid')) {
            $api_url = 'https://graph.microsoft.com/v1.0/sites/' . getConfig('siteid') . '/drive/root';
        } elseif (getConfig('DriveId')) {
            $api_url = 'https://graph.microsoft.com/v1.0/drives/' . getConfig('DriveId') . '/root';
        } else {
        ...
        $client_secret = urlencode($_SERVER['client_secret']);
        $scope = urlencode($_SERVER['scope']);
        $DownurlStrName = '@microsoft.graph.downloadUrl';
        $refresh_token = getConfig('refresh_token');
    }
}

class MS extends MS365 {
    public function __construct()
    {
        $client_id = '734ef928-d74c-4555-8d1b-d942fa0a1a41';
        $client_secret = ':EK[e0/4vQ@mQgma8LmnWb6j4_C1CSIW';
    ...
    public function get_access_token()
    {
        ...
        while ($response['stat']==0&&$p<3) {
            $response = curl_request( $this->oauth_url . 'token', 'client_id='. $this->client_id .'&client_secret='. $this->client_secret .'&grant_type=refresh_token&requested_token_use=on_behalf_of&refresh_token=' . $this->refresh_token );
            $p++;
        }
        if ($response['stat']==200) $ret = json_decode($response['body'], true);
        if (!isset($ret['access_token'])) {
            error_log($this->oauth_url . 'token?client_id='. $this->client_id .'&client_secret='. $this->client_secret .'&grant_type=refresh_token&requested_token_use=on_behalf_of&refresh_token=' . substr($this->refresh_token, 0, 20) . '******' . substr($this->refresh_token, -20));
        ...
        savecache('access_token', $$this->access_token, $_SERVER['disktag'], $ret['expires_in'] - 300);
