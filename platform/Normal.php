...
function getpath()
{
    ...
    if ($noRewrite) $_SERVER['base_path'] .= '?/';
    if (isset($_SERVER['UNENCODED_URL'])) $_SERVER['REQUEST_URI'] = $_SERVER['UNENCODED_URL'];
    if ($noRewrite) {
        $p = strpos($_SERVER['REQUEST_URI'], '?');
        if ($p>0) {
            $path = substr($_SERVER['REQUEST_URI'], $p+1);
            $p = strpos($path, '?');
            if ($p>0) $path = substr($path, 0, $p);
        }
    } else {
        $p = strpos($_SERVER['REQUEST_URI'], '?');
...
function GetGlobalVariable()
{
    ...
    if ($p>0) {
        if ($noRewrite) {
            $tmp = substr($_SERVER['REQUEST_URI'], $p+1);
            $p = strpos($tmp, '?');
            if ($p>0) {
                $getstr = substr($tmp, $p+1);
            }
        } else {
    ...
    foreach ($_POST as $key => $value) {
        if ($key != '') $postarr[urldecode($key)] = urldecode($value);
    }
    if (isset($postarr)) $_POST = $postarr;
}
